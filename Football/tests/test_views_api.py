...
class JoueurViewSetTest(ViewSetCommonTestsMixin, APITestCase):
    ...
    def setUp(self):
        ...
        self.obj = Joueur.objects.create(
            id_J=1, nom=self.nom, age=25, poste="ATT", nationalite="France", pied_fort="D", nombre_but=40
        )
...
class EntraineurViewSetTest(ViewSetCommonTestsMixin, APITestCase):
    ...
    def setUp(self):
        self.nom = "Deschamps"
        self.obj = Entraineur.objects.create(nom=self.nom, experience=10, nationalite="France")
...
class EquipeViewSetTest(ViewSetCommonTestsMixin, APITestCase):
    def setUp(self):
        ...
        entraineur = Entraineur(id_En=1, nom="Deschamps", experience=10, nationalite="France")
        entraineur.save()  # Séparation explicite
        assert entraineur.pk is not None  # Vérifie qu'il a été sauvegardé

        self.obj = Equipe.objects.create(nom=self.nom, stade="Stade de France", entraineur=entraineur)
